...
class Data:
    ...
    def __init__(self):
        ...
        self.df = pd.read_csv('/home/vaibhav/Downloads/archive/drugsComTrain_raw.csv').drop(['uniqueID','date'],axis = 1)
    ...
    def Set_mapping(self):
        ...
        self.Medical_Conditions_name_to_ID =  dict([(value, key) for key, value in Medical_Conditions_ID_to_name.items()])
        self.Drugs_name_to_ID =  dict([(value, key) for key, value in Drugs_ID_to_name.items()])

        self.Medical_Conditions_ID_to_NAME = pd.DataFrame(list(Medical_Conditions_ID_to_name.items()))
